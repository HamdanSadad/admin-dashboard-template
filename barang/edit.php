...
$stmt = mysqli_prepare($connection, "SELECT id, kode_barang, nama_barang, harga FROM `barang` WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        $stmt = mysqli_prepare($connection, "UPDATE `barang` SET `kode_barang` = ?, `nama_barang` = ?, `harga` = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "ssdi", $kode_barang_post, $nama_barang_post, $harga_post, $id);
